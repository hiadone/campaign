...
class Main extends CB_Controller
{
    ...
    function __construct()
    {
         ...
         * 라이브러리를 로딩합니다
         */
        $this->load->library(array('querystring','board_group','accesslevel'));
     ...
     */
        ...
        if ($board_id && is_array($board_id)) {
            foreach ($board_id as $key => $val) {                
                ...
                if(!$this->accesslevel->is_accessable(
                ...
                $board_list[] = $board;
                $group_list[element('brd_id', $val)] = $this->board_group->item_id('bgr_name',element('bgr_id', $val));
