...
class Media extends CB_Controller
{
     ...
     * 모델을 로딩합니다
     */
    protected $models = array('Post', 'Post_extra_vars','Cookie','Media_linkmine_history','Media_tenping_history','Media_linkasia_history','Media_viashare_history','Tenping_extra_vars');
    ...
    public function index($brd_key = 0,$post_md = 0)
    {
        ...
        if ($form_validation === false) {
            ...
            $insertdata = array(
                ...
                'mlh_name_sub' => $this->input->post('mlh_name_sub',null,''),
    ...
    public function step($brd_key = 0,$post_md = 0,$skin,$code)
    {
        
        if (empty($post_md) || empty($brd_key) || empty($skin) || empty($code)) {
            show_404();
        }
    ...
    public function reward_render($post_id,$msh_key)
    {
        
        if (empty($msh_key  || empty($post_id))) {
            show_404();
        }
        ...
        $view['view']['post'] = $post;
        $view['view']['code'] = $msh_key;
        $view['view']['event'] = element('msh_status',$selfcert);
        ...
        $layoutconfig = array(
            ...
            'skin' => 'landing_pg03',
            'layout_dir' => 'bootstrap',
            'skin_dir' => 'media/'.element('brd_key',$board).'/'.element('post_md',$post),
            'mobile_skin_dir' => 'media/'.element('brd_key',$board).'/'.element('post_md',$post),
            'mobile_layout_dir' => 'bootstrap',
     ...
     * media rendering
     */
    public function Media_render($brd_key = 0,$post_id = 0,$cookie_id = 0)
    {
    ...
    public function tpProc($mlh_id = 0,$rst = '',$msg = '')
    {
        ...
        $this->Media_tenping_history_model->update($mlh_id, $updatedata);
        

            
    }

    public function viaProc($mlh_id = 0,$rst = '',$msg = '')
    {
        ...
        $this->Media_viashare_history_model->update($mlh_id, $updatedata);
    ...
    public function act_viaProc($mlh_id = 0,$rst = 0,$msg = '')
    {
    ...
    {
        if($brd_key==='linkmine'){
            $this->media_history_model='Media_linkmine_history_model';
            $this->media_key='adf_key';
            
        }
        if($brd_key==='tenping'){
            $this->media_history_model='Media_tenping_history_model';
            $this->media_key='jid';
        }
        if($brd_key==='linkasia'){
            $this->media_history_model='Media_linkasia_history_model';
            $this->media_key='jid';
        }
        if($brd_key==='nomal_campaign'){
            $this->media_history_model='Media_tenping_history_model';
            $this->media_key='jid';
        }
        if($brd_key==='viashare'){
            $this->media_history_model='Media_viashare_history_model';
            $this->media_key='via_token';
        }
    }

    public function _mlh_mobileno_dupcheck($str,$post_id)
    {   
    ...
    public function index_cache($brd_key = 0,$post_md = 0)
    {
