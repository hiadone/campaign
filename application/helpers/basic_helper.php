...
if ( ! function_exists('admin_listnum_selectbox')) {
    function admin_listnum_selectbox()
    {
        ...
        $html = '<select name="listnum" class="form-control" onchange="location.href=\'' . current_url() . '?listnum=\' + this.value+\'&'.$param->replace('listnum').'\';">';
 ...
 * 나이 얻기
 */
if ( ! function_exists('is_age')) {
    function is_age($birthday = '')
    {
        $birthday = str_replace('-', '', $birthday);
        if (strlen($birthday) !== 8) return false;
        if ( ! is_numeric($birthday)) return false;
